...
var orm_update = {
	...
	edit_set_weight: function(set_id, updated_weight, callback) {
		var query_string = "UPDATE `sets` SET `weight`=" + updated_weight  + " WHERE `set_id`=" + set_id + ";";
		execute(query_string, callback);
	},
	//
	edit_set_sets: function(set_id, updated_no_sets, callback) {
		var query_string = "UPDATE `sets` SET `no_sets`=" + updated_no_sets + " WHERE `set_id`=" + set_id + ";";
		execute(query_string, callback);
	},
	//
	edit_set_reps: function(set_id, updated_no_reps, callback) {
		var query_string = "UPDATE `sets` SET `no_reps`=" + updated_no_reps + " WHERE `set_id`=" + set_id + ";";
		execute(query_string, callback);
	},
	//
	edit_set_rpe: function(set_id, updated_rpe, callback) {
		var query_string = "UPDATE `sets` SET `rpe`=" + updated_rpe + " WHERE `set_id`=" + set_id + ";";
		execute(query_string, callback);
	},
	//
	...
	edit_movement_name: function(movement_id, updated_name, callback) {
		var query_string = 'UPDATE `movements` SET `movement_name`="' + updated_name + '" WHERE `movement_id`=' + movement_id + ';';
		execute(query_string, callback);
	},
	//
	edit_condition_name: function(condition_id, updated_name, callback) {
		var query_string = 'UPDATE `conditions` SET `condition_name`="' + updated_name + '" WHERE `condition_id`=' + condition_id + ';';
		execute(query_string, callback);
	}
// end orm_update
};
